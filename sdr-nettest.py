...
EXPCONFIG = {
        ...
        "nodeid": "local.nodeid",
        "meta_grace": 30,                              # Grace period to wait for interface metadata
        "exp_grace": 200,                               # Grace period before killing experiment
...
def run_exp(meta_info, expconfig):
    ...
    try:
        os.chdir("/opt/monroe/")
        print(check_output(["chmod", "+x", "lime.sh"]))
        print(check_output(["chmod", "+x", "test.sh"]))
        print(check_output(["ls", "-lh"]))

        # proc_sdr = Popen("./test.sh", shell=True, stdout=PIPE)
        # print(proc_sdr.communicate()[0])

        proc_sdr = Popen("./lime.sh", shell=True, stdout=PIPE)
        sdr_outdir = proc_sdr.communicate()[0]
        if isinstance(sdr_outdir,int)==False:
            print("SDR output directory: " + sdr_outdir)
    except Exception as e:
        if cfg['verbosity'] > 0:
            print ("(SDR) Execution or parsing failed for error: {}").format(e)

    #CM: starting Nettest
    if cfg['verbosity'] > 1:
        print("\nDBG: Starting Nettest..")
    try:
    ...
    #TODO 2
    #CM: collecting SDR results
    try:
        shutil.copy(sdr_outdir, '/monroe/results/')
    except Exception as e:
        if cfg['verbosity'] > 0:
            print ("(SDR) Execution or parsing failed for error: {}").format(e)
